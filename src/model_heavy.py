...
class RecursiveProcessing(nn.Module):
    def __init__(self, hidden_size: int, depth: int = 2):
        ...
        self.alpha = nn.Parameter(torch.ones(1) * 6.0)
...
class Crux(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == len(self.layers) // 2:
                processed_recursive = self.recursive(
                    x[:, 4:],
                    context[:, 4:],
                    t_emb=t_emb if timesteps is not None else None,
                )
                x = torch.cat([x[:, :4], processed_recursive], dim=1)
                context = x.clone()
    ...
    def get_colbert_embeddings(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == len(self.layers) // 2:
                processed_recursive = self.recursive(
                    x[:, 4:], context[:, 4:], t_emb=None
                )
                x = torch.cat(
                    [x[:, :4], self.recursive_norm(processed_recursive)], dim=1
                )
                context = x.clone()
