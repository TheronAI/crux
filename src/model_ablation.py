...
class Crux(nn.Module):
    def __init__(
        ...
        num_layers: int = 4,  # Parameter-matched to 27M Light version
        num_heads: int = 8,
        ...
        recursive_depth: int = 0,  # Disabled for ablation
        ...
        # Standard sequential layers (No weight sharing)
        self.layers = nn.ModuleList(
            [
                TransformerLayer(hidden_size, intermediate_size, num_heads, 64, dropout)
                for _ in range(num_layers)
            ]
        )

        self.output_norm = nn.LayerNorm(hidden_size)
        self.recursive = nn.Module()  # Dummy for compatibility
    ...
    def _init_weights(self):
        ...
        for layer in self.layers:
            depth_scale = 1.0 / math.sqrt(2 * self.num_layers)
        ...
        nn.init.normal_(self.diffusion_head[2].weight, std=0.01)
    ...
    def forward(
        ...
        # Sequential Non-Looped Layers
        for layer in self.layers:
            if self.gradient_checkpointing and self.training:
                x, context = checkpoint(layer, x, context, use_reentrant=False)
            else:
...
if __name__ == "__main__":
    model = Crux(vocab_size=50257)
    num_params = sum(p.numel() for p in model.parameters())
    print(f"Total ablation parameters: {num_params:,}")
